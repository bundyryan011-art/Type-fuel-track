            ...
            {PETS.map(function(pet){
              ...
              return(
                <button key={pet.id} onClick={function(){selectPet(pet.id);}} style={{background:isActive?pet.color+"22":T.card2,border:"2px solid "+(isActive?pet.color:T.border),borderRadius:14,padding:"14px",cursor:"pointer",textAlign:"left",display:"flex",alignItems:"center",gap:14,transition:"all .2s"}}>
                  <div style={{flexShrink:0,width:50,display:"flex",alignItems:"center",justifyContent:"center"}}>{stage&&stage.Art?<stage.Art size={44}/>:pet.stages[0].Art?<pet.stages[0].Art size={44}/>:null}</div>
                  <div style={{flex:1}}>
                    ...
                    <div style={{display:"flex",gap:4,alignItems:"center",flexWrap:"wrap"}}>
                      {pet.stages.map(function(s,i){return(<span key={i} title={s.form} style={{opacity:0.8,display:"inline-flex"}}>{s.Art?<s.Art size={22}/>:null}</span>);})}
                    </div>
                    {(pd.xp||0)>0&&(
            ...
            {PETS.map(function(pet){
              ...
              if(unlocked)return(
                <div key={pet.id} style={{background:pet.color+"22",border:"2px solid "+pet.color,borderRadius:14,padding:"16px 12px",textAlign:"center"}}>
                  <div style={{marginBottom:6,display:"flex",justifyContent:"center"}}>{maxStage.Art?<maxStage.Art size={52}/>:null}</div>
                ...
                <div>
                  <div style={{display:"flex",alignItems:"center",gap:16}}>
                    <div className="pf" style={{cursor:"pointer",flexShrink:0}} onClick={function(){setShowPetPick(true);}}>
                      {petStage&&petStage.Art?<petStage.Art size={52}/>:activePet.stages[0].Art?<activePet.stages[0].Art size={52}/>:null}
                    </div>
                    <div style={{flex:1}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:14}}>
                <div style={{background:T.card,border:"1px solid "+T.border,borderRadius:18,padding:"24px 20px",textAlign:"center"}}>
                  <div className="pf" style={{marginBottom:10,display:"flex",justifyContent:"center"}}>
                    {petStage&&petStage.Art?<petStage.Art size={80}/>:activePet.stages[0].Art?<activePet.stages[0].Art size={80}/>:null}
                  </div>
                  ...
                  {activePet.stages.map(function(stage,i){
                    ...
                    return(
                      <div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:i<activePet.stages.length-1?"1px solid "+T.border:"none",opacity:unlocked?1:0.35}}>
                        <div style={{flexShrink:0,width:50,display:"flex",alignItems:"center",justifyContent:"center"}}>{stage.Art?<stage.Art size={44}/>:null}</div>
                        <div style={{flex:1}}>
        ...
        {[
          ...
          return(
            <button key={nav.id} style={navSt(tab===nav.id)} onClick={function(){setTab(nav.id);}}>
              <span style={{fontSize:nav.id==="pet"?0:17,lineHeight:1.2,display:"flex",alignItems:"center",justifyContent:"center"}}>
                {nav.id==="pet"&&nav.icon?<nav.icon size={24}/>:nav.id!=="pet"?nav.icon:activePetId?"●":"○"}
              </span>
              <span>{nav.label}</span>
