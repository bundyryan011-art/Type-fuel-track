...
function StormS4(p){var s=p.size||80;return(<svg width={s} height={s} viewBox="0 0 100 100"><defs><radialGradient id="st4g" cx="50%" cy="50%" r="50%"><stop offset="0%" stopColor="#b45309"/><stop offset="100%" stopColor="#0a0500"/></radialGradient></defs><circle cx="50" cy="50" r="38" fill="url(#st4g)"/><circle cx="50" cy="50" r="28" fill="none" stroke="#ca8a04" strokeWidth="0.8" opacity="0.8"/><polygon points="60,14 44,46 58,46 40,86 72,40 56,40" fill="#fef08a"/><circle cx="40" cy="38" r="5" fill="#fef08a"/><circle cx="58" cy="38" r="5" fill="#fef08a"/><circle cx="40" cy="38" r="2.5" fill="#78350f"/><circle cx="58" cy="38" r="2.5" fill="#78350f"/></svg>);}
          ...
          {scoredMeal.details.length>0&&<div style={{display:"flex",flexDirection:"column",gap:7,marginBottom:16}}>
            {scoredMeal.details.map(function(d,i){return(
              <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:T.card2,borderRadius:12,border:"1px solid "+T.border}}>
                <span style={{fontSize:15}}>{d.pos?"✅":"💡"}</span>
      ...
      {showWorkoutModal&&modal(mBox(
        <div>
          ...
          <div style={{display:"flex",gap:8,marginBottom:20}}>
            {[{id:"light",label:"EASY",emoji:"😊"},{id:"moderate",label:"MEDIUM",emoji:"💪"},{id:"hard",label:"HARD",emoji:"🔥"}].map(function(lvl){var active=wkIntensity===lvl.id;return(
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              <button onClick={function(){setTab("build");}} style={{padding:"13px 14px",background:T.accent,border:"none",borderRadius:14,color:"#030712",cursor:"pointer",display:"flex",alignItems:"center",gap:10,fontFamily:"'Space Mono',monospace",fontSize:11,fontWeight:700,letterSpacing:"0.04em"}}>
                <span style={{fontSize:20}}>🍽️</span> LOG MEAL
              </button>
              <button onClick={function(){setShowWorkoutModal(true);}} style={{padding:"13px 14px",background:"#f97316",border:"none",borderRadius:14,color:"white",cursor:"pointer",display:"flex",alignItems:"center",gap:10,fontFamily:"'Space Mono',monospace",fontSize:11,fontWeight:700,letterSpacing:"0.04em"}}>
                <span style={{fontSize:20}}>🏋️</span> LOG WORKOUT
              </button>
            </div>
            ...
            <div style={card()}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                {(searchR.length>0?searchR:QUICK_CATS[catIdx].items).map(function(food,i){return(
                  <div key={i} className="fc" onClick={function(){addIng({name:food.name,emoji:food.emoji||"🔍",cal:food.cal,pro:food.pro,car:food.car,fat:food.fat,fib:food.fib});}} style={{background:T.card2,border:"1px solid "+T.border,borderRadius:14,padding:"12px"}}>
                    <div style={{fontSize:24,marginBottom:5,lineHeight:1}}>{food.emoji||"🔍"}</div>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:14}}>
                ...
                <div style={card()}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:18}}>
                    {[{label:"Days Logged",value:totalDays,icon:"📅"},{label:"Total XP",value:activePetXP,icon:"⚡"},{label:"Meals Today",value:meals.length,icon:"🍽️"},{label:"Workouts",value:workouts.length,icon:"🏋️"}].map(function(s){return(
                  ...
                  {sLbl("XP GUIDE")}
                  {[["🍽️","Log any meal","+10 XP"],["🏋️","Log a workout","+12 XP"],["💧","Hit water goal","+10 XP"],["🔥","Hit calorie goal","+15 XP"],["💪","Hit protein goal","+20 XP"],["🌟","Amazing meal score","+15 XP"],["🌅","Full day complete","Bonus XP"]].map(function(x){return(
